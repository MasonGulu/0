...
local url = "https://raw.githubusercontent.com/MasonGulu/CCSimpleGUI/master/ccsg/"
...
local function main()
  local widgetLayout = {
    ...
    text.new({1,3},{WIDTH,1},"REQUIRED packages: gui, widget"),
    text.new({1,4},{WIDTH,1},"popup requires other packages."),
    divider.new({1,5},{WIDTH,1})
  }

  for i = 1, #availableWidgets, 2 do
    local valueLeft = availableWidgets[i]
    local valueRight = availableWidgets[i+1]
  ...
  widgetLayout[#widgetLayout+1] = divider.new({1,6+math.ceil(#availableWidgets/2)},{WIDTH,1})
  widgetLayout.cancel = button.new({1,7+math.ceil(#availableWidgets/2)}, {HALFWIDTH, 1}, "Cancel")
  widgetLayout.submit = button.new({HALFWIDTH+1, 7+math.ceil(#availableWidgets/2)}, {HALFWIDTH, 1}, "Install")
  widgetLayout[#widgetLayout+1] = divider.new({1,8+math.ceil(#availableWidgets/2)},{WIDTH,1},{bottom=true})

  local win = gui.new(widgetLayout, {autofit=true})

  while true do
    local event, values = win:read()
    if values.popup then
      -- popup requires some other objects to be installed, ensure they're checked
      for k,v in ipairs(popupRequires) do
    ...
    elseif event == "submit" then
      ...
      return performInstall(values)
    end
  end
end

function performInstall(values)
  local toInstall = {"gui", "widget"}
  for k,v in pairs(availableWidgets) do
    if values[v] then
  ...
  for k,v in ipairs(toInstall) do
    ...
    local file = fs.open("/ccsg/"..v..".lua", "w")
    local web = http.get(url..v..".lua")
    if type(file) == "nil" then
      term.clear()
      term.setCursorPos(1,1)
      error("Unable to open /ccsg/"..v..".lua")
    end
